...
نفترض المشروع في `E:\المشروع` و PHP في `E:\tools\php83`.
...
7. عدّل ملف **`htdocs/index.php`** ليطيبق المسارات الجديدة:
...
```bash
E:\tools\php83\php.exe artisan migrate:fresh --seed --force
E:\tools\php83\php.exe artisan migration:generate --path=database/schema.sql   # إن توفرت الحزمة
```

> الطريقة الأسهل والأنجح مع InfinityFree: استخدم **phpMyAdmin في VistaPanel** وأنشئ الجداول عبر استيراد ملف SQL جاهز. أنشئ الملف محليًا بأي أداة، أو الأبسط: افتح phpMyAdmin على الاستضافة ونفّذ أوامر SQL التي تصدّرها من محليًا بأداة مثل `sqlite3 .dump` محولة لـ MySQL.
...
- **النسخ الاحتياطي**: نزّل نسخة دورية من قاعدة البيانات (phpMyAdmin → Export) وملفات `storage/app/public` (الصور المرفوعة).
...
| الصور المرفوعة لا تظهر | أعِد إنشاء رابط التخزين: أنشئ `setup.php` مؤقت يستدعي `Artisan::call('storage:link')` — أو انسخ `storage/app/public` يدويًا وعدّل المسار |
...
| رفع ملفات كبيرة يفشل | المجاني يحد الرفع ~10MB — قسّم الفيديوهات أو استخدم حجمًا أصغر |
