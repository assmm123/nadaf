...
class TelegramBotService
{
    ...
    public static function poll(): int
    {
        ...
        foreach ($updates as $update) {
            ...
            // حماية: الأوامر للصاحب المتجر فقط
            ...
            $reply = self::handle($chatId, trim($msg['text']), $msg['chat']['username'] ?? null);
    ...
    public static function handle(string $chatId, string $text, ?string $username = null): string
    {
        // تسجيل الرسالة في السجل (مع مرجع التحديث الفعلي إن توفر)
        \App\Models\BotMessageLog::create([
            'update_id' => (int) (microtime(true) * 1000) % 9223372036854775807,
            'chat_id' => (int) $chatId,
            'username' => $username,
            'text' => $text,
            'reply' => null,
    ...
    private static function searchByCode(string $query): string
    {
        ...
        foreach ($variants as $v) {
            ...
            $price = $v->price_usd ?: $v->product->price_usd;
    ...
    public static function handleEditCommand(string $chatId, string $field, string $value, string $code): string
    {
        ...
        return "✏️ مسودة تعديل:\n\n".
            ...
            "الحالي: {$fieldAr} = ".($field === 'price' ? fmt_usd((float) $variant->price_usd) : $variant->quantity)."\n".
            "الجديد: {$fieldAr} = ".($field === 'price' ? fmt_usd((float) $value) : $value)."\n\n".
    ...
    private static function applyPendingEdit(int $chatId, string $text): string
    {
        ...
        $oldValue = $pending->field === 'price' ? $variant->price_usd : $variant->quantity;
        ...
        if ($pending->field === 'price') {
            ...
            $variant->price_usd = round((float) $pending->new_value, 2);
        } else {
        ...
        return "✅ طُبّق التعديل:\n".
            ...
            "{$fieldAr}: {$oldValue} ← ".($pending->field === 'price' ? fmt_usd((float) $variant->price_usd) : $variant->quantity);
    }
